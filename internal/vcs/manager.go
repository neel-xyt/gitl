...
func Firstsetup() {
	...
	if isRealEmail(useremil) {
		...
		// Save next to the binary, in `data/userinfo.json`
		binDir := getBinaryDir()
		configPath := filepath.Join(binDir, "data", "userinfo.json")
		GenerateJSON(finalData, configPath)

		fmt.Println("[+] User configuration saved next to binary at:", configPath)
	} else {
